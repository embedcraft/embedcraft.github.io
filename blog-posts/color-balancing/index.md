<h1 align="center">
 <img src="images/MIPI-Camera.png">
  <br />
...
![Exposure triangle: shutter time, gain/ISO and aperture all feed one EV total, each with its own side-effect](images/setting-exposure-triangle.svg)
...
![EV / stops bar chart: each +1 EV doubles the relative light; the same EV is reachable by time, ISO or aperture](images/setting-ev-stops.svg)
...
![Bit-depth staircase: an ideal smooth ramp versus 3-bit and 5-bit quantisation, showing banding in the coarse case](images/setting-bit-depth.svg)
...
![HDR: long, mid and short sub-exposures each cover part of the scene range; a weighted merge builds a 20-bit linear frame that a tone curve compresses to 8-bit](images/setting-hdr-merge.svg)
...
![White balance: a grey card under 3000K light reads orange in RAW; applying R and B gains restores it to neutral grey](images/setting-white-balance.svg)
...
![Denoise and sharpen on one edge: raw noisy step → denoised (grain gone, step softened) → sharpened (edge steepened with over/undershoot halo)](images/setting-denoise-sharpen.svg)
...
![Autofocus: contrast AF climbs a contrast-vs-position curve and must overshoot the peak (hunting); phase-detect AF reads a left/right pixel disparity that gives focus direction and distance in one step](images/setting-autofocus.svg)
